...
do
  ...
  export THRESHOLD=$THRESH
	runcpu --config=native_cpu_overhead_threshold --noreportable --tune=base --copies=1 --iteration=1 --action=clobber --size=$SIZE $val >> output.txt
	runcpu --config=native_cpu_overhead_threshold --noreportable --tune=base --copies=1 --iteration=$ITER --size=$SIZE $val >> output.txt
