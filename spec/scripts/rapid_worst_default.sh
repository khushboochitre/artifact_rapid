...
do
	...
	runcpu --config=rapid_worst_default --noreportable --tune=base --copies=1 --iteration=1 --action=clobber --size=$SIZE $val >> output.txt
	runcpu --config=rapid_worst_default --noreportable --tune=base --copies=1 --iteration=$ITER --size=$SIZE $val >> output.txt
