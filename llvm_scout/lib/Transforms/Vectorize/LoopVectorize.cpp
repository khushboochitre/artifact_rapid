...
static bool isHandlingCustomLoops = false;
/// @{
...
STATISTIC(CustomLoop, "Number of loops cloned");
STATISTIC(BenLoop, "Number of loops benefitted");
STATISTIC(NotBenLoop, "Number of loops not benefitted");

static cl::opt<bool> AllowCustomVect(
    "allow-custom-vect", cl::init(false));
static cl::opt<bool> DebugModeFile(
  "debug-mode", cl::init(false));
...
void InnerLoopVectorizer::fixFirstOrderRecurrence(PHINode *Phi) {
  ...
  if (LI->getLoopFor(LoopVectorBody)->isLoopInvariant(PreviousLastPart) ||
    ...
    if(PHINode *PrevPHI = dyn_cast<PHINode>(Previous)) {
      bool isPHI = true;
      while(isPHI) { 
        assert(PrevPHI->getParent() == Phi->getParent());
        assert(PrevPHI->getNumIncomingValues() == 2);
        assert(PrevPHI->getBasicBlockIndex(Preheader) >= 0 &&
        ...
        if(isa<Instruction>(PrevPHI->getIncomingValueForBlock(Latch))) {
          if(isa<PHINode>(PrevPHI->getIncomingValueForBlock(Latch))) {
            ...
            auto *PrevPrevious = PrevPHI->getIncomingValueForBlock(Latch);
            Value *PrevPreviousLastPart = getOrCreateVectorValue(PrevPrevious, UF - 1);
            Builder.SetInsertPoint(
              &*++BasicBlock::iterator(cast<Instruction>(PrevPreviousLastPart)));
            isPHI = false;
          }
        }
      }
    ...
    /*if(PHINode *PrevPHI = dyn_cast<PHINode>(Previous)) {
      ...
      assert(PrevPHI->getBasicBlockIndex(Preheader) >= 0 &&
            PrevPHI->getBasicBlockIndex(Latch) >= 0);
      if(PrevPHI->getBasicBlockIndex(Latch) >= 0) {
        auto *PrevPrevious = PrevPHI->getIncomingValueForBlock(Latch);
        Value *PrevPreviousLastPart = getOrCreateVectorValue(PrevPrevious, UF - 1);
        Builder.SetInsertPoint(
        &*++BasicBlock::iterator(cast<Instruction>(PrevPreviousLastPart)));
...
void InnerLoopVectorizer::updateAnalysis() {
  ...
  if(!isHandlingCustomLoops) {
    ...
    DT->recalculate(*LoopExitBlock->getParent());
  }
}
...
bool LoopVectorizePass::isVectorizationPossible(Loop *L, PredicatedScalarEvolution &PSE, DominatorTree *DT,
  ...
  LoopVectorizationLegality LVL(L, PSE, DT, TTI, TLI, AA, F, GetLAA, LI, ORE,
                                R, H, DB, AC);
  bool failedDueToMemoryCheck = false;
  return LVL.canVectorize(false, failedDueToMemoryCheck);
}

bool LoopVectorizePass::processLoop(Loop *L, Loop **NewLoop, 
  BasicBlock **RuntimeChecksBlock, bool allowCloning) {
  ...
  bool failedDueToMemoryCheck = false;
  if (!LVL.canVectorize(EnableVPlanNativePath, failedDueToMemoryCheck)) {
    if(AllowCustomVect && allowCloning && failedDueToMemoryCheck) {
      DenseSet<Instruction *> LoadSet; 
      DenseSet<Instruction *> StoreSet;
      createCloneToVectorize(L, LI, DT, AA, &*NewLoop, 
        &*RuntimeChecksBlock, LoadSet, StoreSet, false);
      if(*NewLoop) {
        CustomLoop++;
...
bool LoopVectorizePass::runImpl(
    ...
    OptimizationRemarkEmitter &ORE_, ProfileSummaryInfo *PSI_, MemoryDependenceResults *MD_) {
  SE = &SE_;
  LI = &LI_;
  TTI = &TTI_;
  DT = &DT_;
  ...
  MD = MD_;
  /*if(DebugModeFile) {
    instrumentWriteFunction(&F, "debug");
  }*/
  ...
  SmallVector<Loop *, 8> NewWorklist;
  DenseMap<Loop *, BasicBlock*> LoopBlockMap;
  DenseMap<unsigned, Loop *> OriginalLoopMap;
  DenseMap<Loop *, unsigned> ClonedLoopMap;
  ...
  while (!Worklist.empty()) {
    ...
    Changed |= processLoop(L, &NewLoop, &RuntimeChecksBlock, true);
    if(NewLoop) {
      assert(RuntimeChecksBlock);
      NewWorklist.push_back(NewLoop);
      LoopBlockMap[NewLoop] = RuntimeChecksBlock;
      /*if(DebugModeFile) {
        OriginalLoopMap[LoopCount] = L;
        ClonedLoopMap[NewLoop] = LoopCount;
        LoopCount++;
      }*/
    }
  }

  assert(NewWorklist.size() == LoopBlockMap.size());
  //LICM.
  for(auto Lp : NewWorklist) {
    Loop *Parent = Lp;
    Loop *Prev = Parent;
    while(Parent) {
      Prev = Parent;
    ...
    LICMPass licm;
    MemorySSA *MSSA;
    licm.callLICM(Lp, AA, LI, DT, TLI, TTI, SE, MSSA, ORE, false);
  }

  //GVN.
  if(NewWorklist.size() > 0) {
    GVN gvn;
    gvn.runImpl(F, *AC, *DT, *TLI, *AA, MD, LI, ORE);
  }
  
  while (!NewWorklist.empty()) {
    isHandlingCustomLoops = true;
    Loop *L = NewWorklist.pop_back_val();
    Changed |=
    ...
    NewChanged = processLoop(L, &NewLoop, &RuntimeChecksBlock, false);
    assert(!NewLoop && !RuntimeChecksBlock);

    Changed |= NewChanged;
    if(!NewChanged) {
      assert(LoopBlockMap[L]);
      restoreLoop(LoopBlockMap[L]);
      /*if(DebugModeFile) {
        assert(VectorLoop == L);
        errs() << "Loops not benefitted " << F.getName() << " " << ClonedLoopMap[L] << "\n";
        instrumentDebugFunction(L, 1, ClonedLoopMap[L], "debug");
      }*/
      NotBenLoop++;
    }
    else {
      assert(LoopBlockMap[L]);
      convertCallToCmp(LoopBlockMap[L]);
      /*if(DebugModeFile) {
        assert(OriginalLoopMap.count(ClonedLoopMap[L]));
        instrumentDebugFunction(OriginalLoopMap[ClonedLoopMap[L]], 0, ClonedLoopMap[L], "debug");
        instrumentDebugFunction(VectorLoop, 1, ClonedLoopMap[L], "debug");
        VectorLoop = nullptr;
      }*/
      BenLoop++;
    }
...
PreservedAnalyses LoopVectorizePass::run(Function &F,
    ...
    bool Changed =
        runImpl(F, SE, LI, TTI, DT, BFI, &TLI, DB, AA, AC, GetLAA, ORE, PSI, MD);
    if (!Changed)
