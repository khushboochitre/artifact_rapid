...
void X86FrameLowering::emitPrologue(MachineFunction &MF,
  ...
  bool UseStackSwitch = ((Fn.getName() == "main") && Fn.hasFnAttribute("switch_stack"));
  ...
  if (has128ByteRedZone(MF) &&
      ...
      !UseStackSwitch &&                        // No stack probes.
  ...
  if (IsWin64Prologue && !IsFunclet && TRI->needsStackRealignment(MF))
	...
	if (UseStackSwitch) {
  	...
  	MachineBasicBlock::iterator ExpansionMBBI = std::prev(MBBI);
		...
		CI = BuildMI(MBB, MBBI, DL, TII.get(X86::CALL64pcrel32))
				.addExternalSymbol(MF.createExternalSymbolName("switch_stack"));
