...
__attribute__((visibility("default"))) size_t __max_global_addr = 0xFFFFFFFFFFFF;

#if 0
static void print_alloc1(size_t total_alloc_sz) {
	static FILE *fp = NULL;
	...
	reenter = 1;
	if (fp == NULL) {
		fp = fopen("/home/piyus/logs/malloc1.txt", "w");
		assert(fp);
	}
	fprintf(fp, "%zd\n", total_alloc_sz);
	reenter = 0;
...
static size_t je_get_size1(void *ptr) {
#if 0
  tsd_t *tsd = tsd_fetch_min();
  tsdn_t *tsdn = tsd_tsdn(tsd);
  assert(tsdn);

	void *ptr_page = (void*)((size_t)ptr & ~0xFFFULL);
...
_je_aligned_alloc(size_t alignment, size_t size) {
	if (alignment > size) {
	...
	if (size >= MIN_LARGE_OBJECT_SIZE) {
		return _san_largealloc(orig_size, idx, NULL);
	}
	...
	//assert(je_get_size1(ret) == san_getsize(ret));
	return ret;
}
...
je_malloc(size_t size) {
	...
	if (size > (MIN_LARGE_OBJECT_SIZE >> 1)) {
		return _je_aligned_alloc(8, size);
	}
	int idx = size_to_idx(size);
	size = 1ULL << idx;
	//return _je_aligned_alloc(size, size);
//#if 0
	print_allocations(size);

	void *ret = _je_malloc(size);
	assert(((size_t)ret & (size - 1)) == 0);
	return ret;
...
je_posix_memalign(void **memptr, size_t alignment, size_t size) {
	memptr[0] = _je_aligned_alloc(alignment, size);
	return 0;
#if 0
	int ret;
	static_opts_t sopts;
...
je_aligned_alloc(size_t alignment, size_t size) {
	return _je_aligned_alloc(alignment, size);
}
...
je_calloc(size_t num, size_t size) {
	...
	if (total_size > (MIN_LARGE_OBJECT_SIZE >> 1)) {
		print_allocations(total_size);
		//print_alloc1(total_size);
		int idx = size_to_idx(total_size);
		//size_t nsize = 1ULL << idx;
		bool Zero = false;
		void *ret = _san_largealloc(total_size, idx, &Zero);
		if (!Zero) {
	...
	void* ret = _je_aligned_alloc(8, total_size);
...
je_realloc(void *ptr, size_t arg_size) {
	if (ptr == NULL) {
		return _je_aligned_alloc(8, arg_size);
		//return _je_malloc(arg_size);
	}
	size_t old_size = san_getsize(ptr);
	if (old_size >= arg_size) {
		return ptr;
	}
	void *ret = _je_aligned_alloc(8, arg_size);
	//void *ret = _je_malloc(arg_size);
	...
	if (likely(ptr != NULL)) {
		...
		if (config_prof && opt_prof) {
			...
			ret = iralloc(tsd, ptr, old_usize, size, size, false,
			    &hook_args);
		}
		tsdn = tsd_tsdn(tsd);
	} else {
	...
	if (config_stats && likely(ret != NULL)) {
		...
		//malloc_printf("usize:%zd isalloc:%zd\n", usize, isalloc(tsdn, ret));
...
_je_free(void *ptr) {
	...
	//assert(je_get_size1(ptr) == san_getsize(ptr));
	LOG("core.free.entry", "ptr: %p", ptr);

	if (san_getsize(ptr) >= MIN_LARGE_OBJECT_SIZE) {
		_san_largefree(ptr);
		return;
	}
