...
extent_alloc_core(tsdn_t *tsdn, arena_t *arena, void *new_addr, size_t size,
    size_t alignment, bool *zero, bool *commit, dss_prec_t dss_prec, szind_t szind) {
	void *ret;
	...
	/* mmap. */
	if ((ret = extent_alloc_mmap(new_addr, size, alignment, zero, commit, szind))
	    != NULL) {
		return ret;
	}
	/* "secondary" dss. */
...
extent_alloc_default_impl(tsdn_t *tsdn, arena_t *arena, void *new_addr,
    size_t size, size_t alignment, bool *zero, bool *commit, szind_t szind) {
	void *ret = extent_alloc_core(tsdn, arena, new_addr, size, alignment, zero,
	    commit, (dss_prec_t)atomic_load_u(&arena->dss_prec,
	    ATOMIC_RELAXED), szind);
...
extent_grow_retained(tsdn_t *tsdn, arena_t *arena,
	...
	if (*r_extent_hooks == &extent_hooks_default) {
		ptr = extent_alloc_default_impl(tsdn, arena, NULL,
		    alloc_size, PAGE, &zeroed, &committed, szind);
	} else {
