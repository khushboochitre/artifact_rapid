...
__attribute__((visibility("default"))) __thread size_t __mi_region = 0;
__attribute__((visibility("default"))) __thread size_t __mi_dummy = 0;
...
mi_heap_t *all_heaps[1000] = {NULL};
mi_heap_t *_mi_heap_init1(int);

extern inline mi_decl_restrict void* mi_region_malloc(size_t size, int region_id) mi_attr_noexcept {
	mi_assert(region_id < 1000);
	if (all_heaps[region_id] == NULL) {
		// FIXME: use locks
		all_heaps[region_id] = _mi_heap_init1(region_id);
	}
	void *ret = mi_heap_malloc(all_heaps[region_id], size);
	//_mi_fprintf(NULL, NULL, "region malloc %p size:%zd r:%d\n", ret, size, region_id);
	return ret;
}


extern inline mi_decl_restrict void* mi_malloc(size_t size) mi_attr_noexcept {
  int region_id = (int)__mi_region;
  __mi_region = 0;
  //if (!(region_id == 0 || region_id == 2 || region_id == 3))
  //  abort();
  //region_id = 0;
  return mi_region_malloc(size, region_id);
...
extern void *large_region_start;
extern void *large_region_end;

// Free a block 
void mi_free(void* p) mi_attr_noexcept
{
	//_mi_fprintf(NULL, NULL, "p:%p start:%p end:%p\n", p, large_region_start, large_region_end);
  mi_segment_t* const segment = mi_checked_ptr_segment(p,"mi_free");
  if (mi_unlikely(segment == NULL)) return; 
	//_mi_fprintf(NULL, NULL, "segment:%p p:%p\n", segment, p);

  mi_threadid_t tid = _mi_thread_id();
  mi_page_t* const page = _mi_segment_page_of(segment, p);
	//_mi_fprintf(NULL, NULL, "page:%p\n", page);
  
  if (mi_likely(tid == mi_atomic_load_relaxed(&segment->thread_id) && page->flags.full_aligned == 0)) {  // the thread id matches and it is not a full page, nor has aligned blocks
    // local, and not full or aligned
    mi_block_t* block = (mi_block_t*)(p);
		//_mi_fprintf(NULL, NULL, "block:%p\n", block);
    if (mi_unlikely(mi_check_is_double_free(page,block))) return;
    mi_check_padding(page, block);
    mi_stat_free(page, block);
    #if (MI_DEBUG!=0)
...
mi_decl_restrict void* mi_calloc(size_t count, size_t size) mi_attr_noexcept {
  ...
  //  abort();
  //region_id = 0;
  return mi_region_calloc(count, size, region_id);
}

extern inline mi_decl_restrict void* mi_region_calloc(size_t count, size_t size, int region_id) mi_attr_noexcept {
	mi_assert(region_id < 1000);
	if (all_heaps[region_id] == NULL) {
		// FIXME: use locks
		all_heaps[region_id] = _mi_heap_init1(region_id);
	}
	void *ret = mi_heap_calloc(all_heaps[region_id], count, size);
	//_mi_fprintf(NULL, NULL, "region calloc %p size:%zd r:%d\n", ret, size*count, region_id);
	return ret;
}
...
extern inline mi_decl_restrict void* mi_region_realloc(void *p, size_t size, int region_id) mi_attr_noexcept {
	mi_assert(region_id < 1000);
	if (all_heaps[region_id] == NULL) {
		// FIXME: use locks
		all_heaps[region_id] = _mi_heap_init1(region_id);
	}
	void *ret = mi_heap_realloc(all_heaps[region_id], p, size);
	//_mi_fprintf(NULL, NULL, "region realloc %p size:%zd r:%d\n", ret, size, region_id);
	return ret;
}
...
void* mi_realloc(void* p, size_t newsize) mi_attr_noexcept {
  ...
  //  abort();
  //region_id = 0;
  return mi_region_realloc(p, newsize, region_id);
}
...
__attribute__((visibility("default"))) void obstack_free(struct obstack *h, void *obj) mi_attr_noexcept 
{
	//struct obstack *h = (struct obstack*)_h;
  register struct _obstack_chunk *lp;	/* below addr of any objects in this chunk */
  register struct _obstack_chunk *plp;	/* point to previous chunk if any */

  lp = (struct _obstack_chunk*)(h->chunk);
  ...
  while (lp != 0)
    {
			char *limit = lp->limit;
			if (!((char*)lp >= (char*)obj || limit < (char*)obj)) {
				break;
			}
      plp = (struct _obstack_chunk*)(lp->prev);
			if (lp >= large_region_start && lp < large_region_end) {
      	mi_free(lp);
			}
			else {
      	CALL_FREEFUN (h, lp);
...
#define DEFAULT_STACK_SIZE (1ULL << 18)

//__thread void* stackptr[16] = {NULL};

__attribute__((visibility(
    "default"))) __thread void* stackptr_s[16] = {NULL};

//__attribute__((visibility(
//    "default"))) __thread void* stackptr_e[16] = {NULL};

static void mi_stack_init(int region_id) mi_attr_noexcept 
{
	mi_assert_internal(region_id < 16 && stackptr_s[region_id] == NULL);
	stackptr_s[region_id] = mi_region_malloc(DEFAULT_STACK_SIZE, region_id);
	//stackptr[region_id] = stackptr_s[region_id];
	//stackptr_e[region_id] = stackptr_s[region_id] + DEFAULT_STACK_SIZE;
	//_mi_fprintf(NULL, NULL, "region stack %p r:%d region:%p oth:%p oth0:%p\n", 
	//		stackptr_s[region_id], region_id, region, &stackptr_s[region_id], stackptr_s);
	//return stackptr_s[region_id];
}

#if 0
#include <assert.h>
#define ASSERT(x) ({ if (!(x)) abort(); })
__attribute__((visibility("default"))) void mi_stack_verify(void *start, void *cur, void *end, int region_id, int alignment)
{
	//_mi_fprintf(NULL, NULL, "start:%p ptr:%p end:%p align:%d region_id: %d start:%p end:%p\n", 
	//		start, cur, end, alignment, region_id, stackptr_s[region_id], stackptr_e[region_id]);
	ASSERT(start <= cur && cur < end);
	ASSERT((((size_t)cur) & (alignment-1)) == 0);
	ASSERT(end <= stackptr_e[region_id]);
	ASSERT(stackptr_s[region_id] == end);
	ASSERT(start >= stackptr[region_id]);
}
#endif

__attribute__((visibility("default"))) void mi_init_stack(int max_region) mi_attr_noexcept 
{
	int i;
	for (i = 2; i <= max_region; i++) {
		mi_stack_init(i);
	}
}
