...
extern __thread size_t __mi_region;
...
int mi_posix_memalign(void** p, size_t alignment, size_t size) mi_attr_noexcept {
  ...
  __mi_region = 0;
	return mi_region_posix_memalign(p, alignment, size, region_id);
#if 0
...
int mi_region_posix_memalign(void** p, size_t alignment, size_t size, int region_id) mi_attr_noexcept {
  // Note: The spec dictates we should not modify `*p` on an error. (issue#27)
  // <http://man7.org/linux/man-pages/man3/posix_memalign.3.html>
  if (p == NULL) return EINVAL;
  if (alignment % sizeof(void*) != 0) return EINVAL;                   // natural alignment
  if (alignment==0 || !_mi_is_power_of_two(alignment)) return EINVAL;  // not a power of 2
  void* q = mi_region_aligned_alloc(alignment, size, region_id);
  if (q==NULL && size != 0) return ENOMEM;
  mi_assert_internal(((uintptr_t)q % alignment) == 0);
  *p = q;
  //_mi_fprintf(NULL, NULL, "region pos_aalloc %p size:%zd r:%d\n", q, size, region_id);
  return 0;
}
...
mi_decl_nodiscard mi_decl_restrict void* mi_aligned_alloc(size_t alignment, size_t size) mi_attr_noexcept {
  ...
  int region_id = (int)__mi_region;
  __mi_region = 0;
	return mi_region_aligned_alloc(alignment, size, region_id);
	#if 0
