...
{
	//_mi_fprintf(NULL, NULL, "init_segment\n");
  ...
  if (segment==NULL) {
    ...
    if (segment==NULL) {
      segment = (mi_segment_t*)_mi_arena_alloc_aligned(segment_size, MI_SEGMENT_ALIGN, &commit, &mem_large, &is_pinned, &is_zero, &memid, os_tld);
			//_mi_fprintf(NULL, NULL, "allocating segment:%p\n", segment);
...
{
	 //_mi_fprintf(NULL, NULL, "heap:%p htld:%p tld:%p\n", heap, heap->tld, tld);
