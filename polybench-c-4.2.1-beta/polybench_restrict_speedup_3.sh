#!/bin/bash
export COMPILER_PATH=/usr/lib/gcc/x86_64-linux-gnu/9:$COMPILER_PATH

POLYBENCH=$HOME/artifact_rapid/polybench-c-4.2.1-beta_1
LLVM_ROOT_PATH=$HOME/artifact_rapid/llvm-3.6.0/build
...
do
    ...
    then
        ...
        if [ $val == "jacobi-1d" ];
        then
            $CLANG $NATIVE_FLAGS -mllvm -handle-higher-order -I $POLYBENCH/utilities -I $POLYBENCH/stencils/$val $POLYBENCH/utilities/polybench.c $POLYBENCH/stencils/$val/$val.c $RESTRICT -DPOLYBENCH_TIME -o $EXE_FILES/$val -lm $GCC_PATH
        else
