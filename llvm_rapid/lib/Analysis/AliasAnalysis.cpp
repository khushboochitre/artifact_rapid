...
static AliasResult checkAliasMetadata(Function *F, 
    ...
    auto Base1 = GetUnderlyingObject(V1, DL);
    auto Base2 = GetUnderlyingObject(V2, DL);
    ...
    if(I1) {
      auto B1 = I1->getParent();
      assert(B1);
      for (BasicBlock::iterator I = B1->begin(); I != B1->end(); ++I) {
        Instruction *Instr = &*I;
        ...
        if (CI && CI->getIntrinsicID() == Intrinsic::custom_noalias) {
          ...
          if (VV1 && VV2 && ((Base1 == VV1->getValue() && Base2 == VV2->getValue()) 
                    || (Base2 == VV1->getValue() && Base1 == VV2->getValue()))) {
            found1 = true;
          }
        }
      }
...
AliasResult AAResults::alias(const MemoryLocation &LocA,
  ...
  if(LocA.Ptr != LocB.Ptr) {
    if(!getDisableCustomIntrinsicChecks()) {
      Instruction *I1 = dyn_cast<Instruction>(const_cast<Value *>(LocA.Ptr));
      Instruction *I2 = dyn_cast<Instruction>(const_cast<Value *>(LocB.Ptr));
      if (I1) {
        if (I1->getFunction()) {
          Function *F = I1->getFunction();
          if(F && F->hasFnAttribute("custom_noalias_intrinsic")) {
            auto Result = checkAliasMetadata(F, LocA, LocB);
            if (Result != MayAlias)
              return Result;
          }
        }
