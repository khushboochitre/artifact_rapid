...
bool RecurrenceDescriptor::isFirstOrderRecurrence(
  ...
  for (User *U : Phi->users())
    if (auto *I = dyn_cast<Instruction>(U)) {
      if(PHINode *UsePHI = dyn_cast<PHINode>(I)) {
        std::vector<Instruction *> Visited;
        std::vector<PHINode *> Worklist;
        Worklist.push_back(UsePHI);
        while(!Worklist.empty()) {
          PHINode *UUsePHI = Worklist.back();
          Worklist.pop_back();
          if (UUsePHI->getParent() == Phi->getParent() &&
              UUsePHI->getNumIncomingValues() == 2 &&
              UUsePHI->getBasicBlockIndex(Preheader) >= 0 &&
              UUsePHI->getBasicBlockIndex(Latch) >= 0) {
            for (User *UU : UUsePHI->users()) { 
              if (auto *UI = dyn_cast<Instruction>(UU)) {
                if(PHINode *UUUsePHI = dyn_cast<PHINode>(UI)) {
                  std::vector<Instruction *>::iterator it = std::find (Visited.begin(), 
                    Visited.end(), UI);
                  if(it == Visited.end()) {
                    Worklist.push_back(UUUsePHI);
                    Visited.push_back(UI);
                  }
                  else {
                    //Cycle exists.
                    return false;
                ...
                else if (!DT->dominates(Previous, UI)) {
                  return false;
                }
              }
            }
      ...
      else if (!DT->dominates(Previous, I)) //&& !isa<PHINode>(I)) 
        return false;
    }
