...
static cl::opt<bool> EnableHighOrd("handle-higher-order",
...
void GVN::fixIRForVectorization(Function *F, PHINode *PHI) {
...
bool GVN::PerformLoadPRE(LoadInst *LI, AvailValInBlkVect &ValuesPerBlock,
  ...
  for (const auto &PredLoad : PredLoads) {
    ...
    assert(isa<Instruction>(LI));
    if (MDNode* N = (*LI).getMetadata("Inst_ID")) {
      unsigned ID = stoull(dyn_cast<MDString>(N->getOperand(0))->getString().str());
      (*NewLoad).setMetadata("Inst_ID", N);
    }
  ...
  bool found = false;
  if(PHINode *PHI1 = dyn_cast<PHINode>(V)) {
    for (Value::user_iterator i = V->user_begin(), e = V->user_end(); i != e; ++i) {
      if (PHINode *PHI2 = dyn_cast<PHINode>(*i)) {
        if(PHI1->getParent() == PHI2->getParent()) {
          found = true;
          break;
        }
      }
    ...
    if(found) {
      BasicBlock *CurBB = PHI1->getParent();
      Instruction *I = dyn_cast<Instruction>(V);
      CurBB->getInstList().remove(I);
      I->insertBefore(CurBB->getFirstNonPHI());
    }
  }

  if(Instruction *I = dyn_cast<Instruction>(V)) {
    AddedPhiNodes.push_back(I);
  }
...
static void patchAndReplaceAllUsesWith(Instruction *I, Value *Repl) {
  ...
  if(Instruction *II = dyn_cast<Instruction>(Repl)) {
    if (MDNode* N = (*I).getMetadata("Inst_ID")) {
      unsigned ID = stoull(dyn_cast<MDString>(N->getOperand(0))->getString().str());
...
bool GVN::processLoad(LoadInst *L) {
  ...
  if (AnalyzeLoadAvailability(L, Dep, L->getPointerOperand(), AV)) {
    ...
    if(Instruction *II = dyn_cast<Instruction>(AvailableValue)) {
      if (MDNode* N = (*II).getMetadata("Inst_ID")) {
        unsigned ID = stoull(dyn_cast<MDString>(N->getOperand(0))->getString().str());
        (*L).setMetadata("Inst_ID", N);
      }
    }
...
bool GVN::runImpl(Function &F, AssumptionCache &RunAC, DominatorTree &RunDT,
  ...
  for(auto Inst : AddedPhiNodes) {
    if(PHINode *PHI1 = dyn_cast<PHINode>(Inst)) {
      if(PHI1->hasNUsesOrMore(1)) {
        fixIRForVectorization(PHI1->getParent()->getParent(), PHI1);
      }
    }
  }
  AddedPhiNodes.clear();
...
bool GVN::processBlock(BasicBlock *BB) {
  ...
  for (BasicBlock::iterator BI = BB->begin(), BE = BB->end();
    ...
    for (auto *I : InstrsToErase) {
      ...
      salvageDebugInfo(*I);
      if (MD) {
        std::vector<Instruction *>::iterator it = std::find (AddedPhiNodes.begin(), AddedPhiNodes.end(), I);
        if(it != AddedPhiNodes.end()) {
          AddedPhiNodes.erase(it);
        }
...
bool GVN::performScalarPRE(Instruction *CurInst) {
  ...
  if (MD) {
    std::vector<Instruction *>::iterator it = std::find (AddedPhiNodes.begin(), AddedPhiNodes.end(), CurInst);
    if(it != AddedPhiNodes.end()) {
      AddedPhiNodes.erase(it);
    }
