...
class LoopPromoter : public LoadAndStorePromoter {
  ...
  ICFLoopSafetyInfo &SafetyInfo;
  unsigned InstID;
  DenseMap<Instruction *, unsigned> InstIDMap;
...
public:
  LoopPromoter(Value *SP, ArrayRef<const Instruction *> Insts, SSAUpdater &S,
               ...
               DenseMap<Instruction *, unsigned> *InstIDMap = nullptr)
      : LoadAndStorePromoter(Insts, S), SomePtr(SP), PointerMustAliases(PMA),
        ...
        SafetyInfo(SafetyInfo), InstIDMap(*InstIDMap) {}
  ...
  void doExtraRewritesBeforeFinalDeletion() override {
    ...
    for (unsigned i = 0, e = LoopExitBlocks.size(); i != e; ++i) {
      ...
      for(auto Inst : InstIDMap) {
        if(StoreInst *SI = dyn_cast<StoreInst>(Inst.first)) {
          if(Ptr == SI->getPointerOperand()) {
            MDNode* N = MDNode::get(NewSI->getContext(), MDString::get
              (NewSI->getContext(), std::to_string(Inst.second)));
            (*NewSI).setMetadata("Inst_ID", N);
          }
        }
      }
...
bool llvm::promoteLoopAccessesToScalars(
  ...
  for (Value *ASIV : PointerMustAliases) {
    ...
    for (User *U : ASIV->users()) {
      ...
      if (MDNode* N = (*UI).getMetadata("Inst_ID")) {
        unsigned ID = stoull(dyn_cast<MDString>(N->getOperand(0))->getString().str());
        if(InstIDMap.count(UI)) {
          assert(InstIDMap[UI] == ID);
        }
        else {
          InstIDMap[UI] = ID;
        }
  ...
  LoopPromoter Promoter(SomePtr, LoopUses, SSA, PointerMustAliases, ExitBlocks,
                        ...
                        Alignment, SawUnorderedAtomic, AATags, *SafetyInfo, &InstIDMap);
  ...
  for(auto Inst : InstIDMap) {
    if(LoadInst *LI = dyn_cast<LoadInst>(Inst.first)) {
      if(SomePtr == LI->getPointerOperand()) {
        MDNode* N = MDNode::get(PreheaderLoad->getContext(), MDString::get
          (PreheaderLoad->getContext(), std::to_string(Inst.second)));
        (*PreheaderLoad).setMetadata("Inst_ID", N);
      }
    }
  }
...
bool LICMPass::callLICM(Loop *L, AliasAnalysis *AA, LoopInfo *LI, DominatorTree *DT,
                TargetLibraryInfo *TLI, TargetTransformInfo *TTI,
                ScalarEvolution *SE, MemorySSA *MSSA,
                OptimizationRemarkEmitter *ORE, bool DeleteAST) {
  LoopInvariantCodeMotion LICM(100, 250);
  return LICM.runOnLoop(L, AA, LI, DT, TLI, TTI, SE, MSSA, ORE, true);
}
