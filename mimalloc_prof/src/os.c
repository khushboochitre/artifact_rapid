...
#define PAGE_SIZE 4096
  if (((size_t)addr & (PAGE_SIZE-1)) != 0) {
	  abort();
  }
  err = (madvise(addr, size, MADV_DONTNEED) == -1);
  if (err) {
...
__attribute__((visibility("default"))) void *large_region_start = NULL;
__attribute__((visibility("default"))) void *large_region_end = NULL;
...
static void* mi_os_mem_alloc(size_t size, size_t try_alignment, bool commit, bool allow_large, bool* is_large, mi_stats_t* stats) {
	static void* region_start[MAX_REGIONS] = {NULL};
	static void* region_end[MAX_REGIONS] = {NULL};
	int region_id = stats->region_id;

	if (try_alignment > MI_SEGMENT_ALIGN) {
		_mi_fprintf(NULL, NULL, "alignment is more than segment\n");
		abort();
	}

	//_mi_fprintf(NULL, NULL, "size:%zd commit:%d id:%d\n", size, commit, stats->region_id);
	if (large_region_start == NULL) {
		size_t padding = (MI_SEGMENT_ALIGN1 * MAX_REGIONS);
		size_t large_size = (padding + (MAX_REGIONS * MI_SEGMENT_ALIGN1 * 20));
    large_region_start = mi_unix_mmap(NULL, large_size, try_alignment, PROT_NONE, false, false, is_large);
		//_mi_fprintf(NULL, NULL, "region_start:%p\n", large_region_start);
		if (large_region_start == NULL) {
			_mi_fprintf(NULL, NULL, "mmap returns NULL\n");
			abort();
			return NULL;
		}
		int i;
		region_start[0] = large_region_start;
		region_end[0] = region_start[0] + (MI_SEGMENT_ALIGN1 * 20);

		for (i = 1; i < MAX_REGIONS; i++) {
			region_start[i] = region_end[i-1] + MI_SEGMENT_ALIGN1;
			region_end[i] = region_start[i] + (MI_SEGMENT_ALIGN1 * 20);
		}
		large_region_end = large_region_start + large_size;
	}
	if (region_id >= MAX_REGIONS) {
		_mi_fprintf(NULL, NULL, "region_id > 100\n");
		abort();
		return NULL;
	}

	//_mi_fprintf(NULL, NULL, "start:%p end:%p size:%zx left:%zx\n", 
		//region_start[region_id], region_end[region_id], size, region_end[region_id] - region_start[region_id]);
	unsigned long long ret = (unsigned long long)region_start[region_id];
	if (try_alignment > 1) {
		ret = (ret + try_alignment - 1) & ~(try_alignment - 1);
	}
	region_start[region_id] = (void*)(ret + size);
	if (region_start[region_id] > region_end[region_id]) {
		_mi_fprintf(NULL, NULL, "region_start:%p region_end:%p region:%d\n", region_start[region_id], region_end[region_id], region_id);
		abort();
		return NULL;
	}
	if (commit) {
    int ret1 = mprotect((void*)ret, size, PROT_WRITE|PROT_READ);
		if (ret1 == -1) {
			_mi_fprintf(NULL, NULL, "ret:%p\n", ret);
			abort();
		}
	}
	//_mi_fprintf(NULL, NULL, "seg:%p id:%d\n", ret, stats->region_id);
	return (void*)ret;
...
static void* mi_os_mem_alloc_aligned(size_t size, size_t alignment, bool commit, bool allow_large, bool* is_large, mi_stats_t* stats) {
	//_mi_fprintf(NULL, NULL, "size:%zx align:%zx\n", size, alignment);
...
void* _mi_os_alloc_aligned(size_t size, size_t alignment, bool commit, bool* large, mi_stats_t* tld_stats)
{
  ...
  return mi_os_mem_alloc_aligned(size, alignment, commit, allow_large, (large!=NULL?large:&allow_large), tld_stats);
}
