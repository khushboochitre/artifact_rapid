...
extern mi_heap_t *all_heaps[1000];
mi_heap_t *_mi_heap_init1(int);

mi_decl_restrict void* mi_region_aligned_alloc(size_t alignment, size_t size, int region_id) mi_attr_noexcept {
  size_t padding = (alignment > 8) ? alignment : 8;
  size += padding;
	...
	void *ret = mi_heap_malloc_aligned(all_heaps[region_id], size, alignment);
	//_mi_fprintf(NULL, NULL, "region aalloc %p size:%zd r:%d\n", ret, size, region_id);
	return ret + padding;
}
